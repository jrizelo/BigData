...
def sim_manhattan(avaliacoes, c1, c2):
	...
	dist_manhattan = sum([abs(avaliacoes[c1][filme] - avaliacoes[c2][filme])
...
def sim_euclidiana(avaliacoes, c1, c2):
	...
	for filme in avaliacoes[c1]:
		if filme in avaliacoes[c2]:
			mesmos_filmes[filme] = 1
	if len(mesmos_filmes) == 0: return 0
	dist_euclidiana = sqrt(sum([pow((avaliacoes[c1][filme] 
	                                 - avaliacoes[c2][filme]), 2)
	                                 for filme in mesmos_filmes]))
...
def sim_pearson(avaliacoes, c1, c2):
	...
	numerador = sum([((avaliacoes[c1][filme]-media_c1) 
	                   * (avaliacoes[c2][filme]-media_c2)) 
					   for filme in mesmos_filmes])
	denominador = sqrt(sum([pow((avaliacoes[c1][filme]-media_c1), 2) 
					   for filme in mesmos_filmes]) 
...
def top_similares(avaliacoes, pessoa, n=1, medida=sim_euclidiana):
	valores = [(medida(avaliacoes, pessoa, outra), outra)
				  	   for outra in avaliacoes if outra != pessoa]
	valores.sort()
	valores.reverse()
	return valores[0:n]
